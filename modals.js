...
const MODALS = {
  ...
  async saveEdit() {
    ...
    UI.setSync('busy');
    try {
      await API.updateStore(upd, STATE.currentUser.name);
      Object.assign(r, upd);
  ...
  async saveAddStore() {
    ...
    UI.setSync('busy');
    try {
      const res = await API.addStore(s, STATE.currentUser.name);
      s.id = res.id;
  ...
  async saveAddUser() {
    ...
    UI.setSync('busy');
    try {
      const res = await API.addUser({ username,password,role,name,ref,status }, STATE.currentUser.name);
  ...
  async saveEditUser(id) {
    ...
    UI.setSync('busy');
    try {
      await API.updateUser({ id,name,role,status }, STATE.currentUser.name);
      const u = STATE.db.users.find(x=>x.id===id);
  ...
  async saveResetPassword(id) {
    ...
    UI.setSync('busy');
    try {
      await API.changePassword(id, '', newPass, STATE.currentUser.name);
      const u = STATE.db.users.find(x=>x.id===id);
